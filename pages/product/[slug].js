...
const Slug = ({ addToCart, product, variants, buyNow, error }) => {
  ...
  const checkPincode = () => {
    if (Object.keys(Pincodes).includes(pin)) {
      setCheckpin(true);
      toast.success("Success Pincode matched!", {
      ...
      setCheckpin(false);
      toast.error("Sorry. Pincode not matched !", {
        position: "bottom-center",
        autoClose: 1000,
        hideProgressBar: false,
        closeOnClick: true,
            ...
            <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
              ...
              <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
                <div className="flex">
                  ...
                  {Object.keys(variants).includes("white") &&
                    Object.keys(variants["white"].includes(size)) && (
                      <button
                        className={`border-2 bg-white-700  ${
                  ...
                  {Object.keys(variants).includes("yellow") &&
                    Object.keys(variants["yellow"]).includes(size) && (
                      <button
                        className={`border-2 bg-yellow-500  ${
                          color === "yellow"
                            ? "border-black"
                            : "border-gray-300"
                        } rounded-full w-6 h-6 focus:outline-none`}
                  ...
                  {Object.keys(variants).includes("purple") &&
                    Object.keys(variants["purple"]).includes(size) && (
                      <button
                        className={`border-2 bg-purple-700  ${
                          color === "purple"
                            ? "border-black"
                            : "border-gray-300"
                        } rounded-full w-6 h-6 focus:outline-none`}
                ...
                <div className="flex ml-6 items-center">
                  ...
                  <div className="relative">
                    ...
                    >
                      {color && Object.keys(variants[color]).includes("SM") && (
              ...
              <div className="flex">
                {product.availableQty > 0 && (
                  <span className="title-font font-medium text-2xl text-gray-900 mx-3 md:mx-0">
                    ${product.price}
                  </span>
                )}
                {product.availableQty <= 0 && (
                ...
                <button
                  ...
                  className="flex ml-2 md:text-md disabled:bg-pink-300 md:ml-1 text-white bg-pink-600 border-0 py-2 px-1 md:px-3 focus:outline-none hover:bg-pink-600 rounded"
                >
                  Buy Now
                </button>
                <button
                  disabled={product.availableQty <= 0}
                  className="flex ml-4 md:text-md disabled:bg-pink-300 text-white bg-pink-600 border-0 py-2 px-1 md:px-3 focus:outline-none hover:bg-pink-600 rounded"
                  onClick={() => {
                    addToCart(
                      slug,
                      1,
              ...
              <div className="pin flex mt-5">
                ...
                />
                <button
                  className="flex ml-4 md:text-md text-white bg-pink-600 border-0 pt-2 px-4 focus:outline-none hover:bg-pink-600 rounded"
              ...
              <div className="message mt-2">
                {!checkpin && checkpin != null && (
                  <div>
                    <p className="text-red-500">
                      Sorry! not serving this pincode currently
                    </p>
                  </div>
                )}
                {checkpin && checkpin != null && (
                  <div>
                    <p className="text-green-500">Yayy! Pincode has matched </p>
                  </div>
                )}
              </div>
            </div>
